...
# Test data
dt <- data.table(col1 = 1:3, col2 = c(4, 4, 5), col3 = c(5, 5, 5))


# Tests ------------------------------------------------------------------------
test_that("sql_select() handles basic column selection", {
    query <- sql(select("col1", "col3"), from("dt"))
...
test_that("sql_select() handles distinct", {
    query <- sql(select("col3", .distinct = TRUE), from("dt"))
    target <- unique(dt[, list(col3)])

    expect_equal(.sqldf(query), target)

    query <- sql(select(vars(col1, col3), .distinct = TRUE), from("dt"))
...
test_that("sql_select() handles aliased columns", {
    query <- sql(select(NewCol = "col1"), from("dt"))
